...
class MCRFMClassifier(nn.Module):
    ...
    def branch_gates(self, zh: torch.Tensor, ze: torch.Tensor, c: float, geometry_mode: str) -> torch.Tensor:
        mode = geometry_mode.lower()
        if mode == "hyperbolic":
        ...
        if self.branch_gate_delta is None:
            return self.branch_gate_prior().to(device=zh.device, dtype=ze.dtype if ze.numel() > 0 else zh.dtype).expand(zh.shape[0], 1)
    ...
    def branch_multipliers(
        ...
        gate = self.branch_gates(zh, ze, c=c, geometry_mode=geometry_mode)
        mode = geometry_mode.lower()
        if mode == "hyperbolic":
    ...
    def beta_values(self, zh: torch.Tensor, ze: torch.Tensor, c: float, geometry_mode: str) -> torch.Tensor:
        if self.mode != "hybrid":
            return torch.ones((zh.shape[0], 1), device=zh.device, dtype=ze.dtype if ze.numel() > 0 else zh.dtype)
        if self.beta_delta is None:
            return self.beta().to(device=zh.device, dtype=ze.dtype if ze.numel() > 0 else zh.dtype).expand(zh.shape[0], 1)
        feats = self._gating_input(zh, ze, c=c, geometry_mode=geometry_mode)
        raw = self.raw_beta.to(device=feats.device, dtype=feats.dtype) + self.beta_delta(feats).squeeze(-1)
        return torch.sigmoid(raw).unsqueeze(-1)

    def feature_representation(self, zh: torch.Tensor, ze: torch.Tensor, c: float, geometry_mode: str) -> torch.Tensor:
        xh, ze = self._normalized_branch_features(zh, ze, c=c)
        mult_h, mult_e = self.branch_multipliers(zh, ze, c=c, geometry_mode=geometry_mode)
        xh = xh * mult_h.sqrt()
        ze = ze * mult_e.sqrt()
        return torch.cat([xh, ze], dim=-1)
    ...
    def linear_logits(self, zh: torch.Tensor, ze: torch.Tensor, c: float, geometry_mode: str) -> torch.Tensor:
        feats = self.feature_representation(zh, ze, c=c, geometry_mode=geometry_mode)
    ...
    def forward(
        ...
        proto = self.prototype_logits(zh, ze, proto_h, proto_e, c=c, geometry_mode=geometry_mode)
        if self.mode == "prototype":
            return proto
        linear = self.linear_logits(zh, ze, c=c, geometry_mode=geometry_mode)
        if self.mode == "linear":
            return linear
        beta = self.beta_values(zh, ze, c=c, geometry_mode=geometry_mode).to(dtype=linear.dtype, device=linear.device)
